...
struct enactor_t : gunrock::enactor_t<problem_t> {
  ...
  using csr_t = typename graph::
      graph_csr_t<memory_space_t::device, vertex_t, edge_t, weight_t>;
  using csc_t = typename graph::
      graph_csc_t<memory_space_t::device, vertex_t, edge_t, weight_t>;
  ...
  void loop(gcuda::multi_context_t& context) override {
    ...
    //auto backward_back = [G, distances, iteration, keep_going,
    //                      new_distances] __device__(vertex_t const& v) -> void {
    ...
    //    auto old_distance = math::atomic::min(&distances[u], iteration + 1);
    //    return (iteration + 1 < old_distance);
    //  }
    //};

    auto search = [distances, single_source, iteration, G] __host__ __device__(
                      vertex_t const& vertex,
                      vertex_t const& parent,
                      edge_t const& edge,
      ...
      auto old_distance = math::atomic::min(&distances[parent], iteration + 1);
      return (iteration + 1 < old_distance);
    };

    operators::advance::execute<operators::load_balance_t::block_mapped,
                                operators::advance_direction_t::backward>(
        G,  // graph
        E,
